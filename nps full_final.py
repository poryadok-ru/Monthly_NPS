...
def add_totals_row(pivot, base_cols, years_order):
    good_cols = [f"Хорошая {y}" for y in years_order if f"Хорошая {y}" in pivot.columns]
    bad_cols = [f"Плохая {y}" for y in years_order if f"Плохая {y}" in pivot.columns]
    neut_cols = [f"Нейтральная {y}" for y in years_order if f"Нейтральная {y}" in pivot.columns]
    cnt_cols = []
    for met in ["Количество отзывов", "Всего"]:
        cnt_cols.extend([f"{met} {y}" for y in years_order if f"{met} {y}" in pivot.columns])
    nps_cols = [f"NPS {y}" for y in years_order if f"NPS {y}" in pivot.columns]
    itog_row = {}
...
def npsview_one(excel_file, add_message):
    ...
    try:
        ...
        # === Диагностика и форматирование Excel ===
        wb = load_workbook(output_file)
        try:
            if "по кодам" in wb.sheetnames:
                sh = wb["по кодам"]
                # print(f"# DIAG: Лист 'по кодам' - всего строк: {sh.max_row}, столбцов: {sh.max_column}")
                ...
                # print(f"# DIAG: Форматирую заголовки листа...")
                ...
                # print(f"# DIAG: Форматирую колонку NPS...")
                ...
                # Покраска жёлтых строк
                # print(f"# DIAG: Начинаю покраску желтых строк...")
                painted = 0
                for i in range(2, max_row + 1):
                    if is_yellow_row2[i - 1]:
                        try:
                            ...
                            painted += 1
                            if painted % 1000 == 0:
                                print(f"# DIAG: Желтых строк покрашено: {painted}/{max_row-1}")
                        except Exception as e:
                            print(f"DIAG Ошибка при покраске строки {i}: {e}")
                            print(traceback.format_exc())
                # print(f"# DIAG: Всего желтых строк покрашено: {painted}")

                # Группировка строк
                # print(f"# DIAG: Начинаю группировку строк...")
                i = 2
                grouped = 0
                while i <= max_row:
                    if is_code_row2[i - 1]:
                        ...
                        if group_end > group_start:
                            try:
                                ...
                                grouped += 1
                                if grouped % 500 == 0:
                                    print(f"# DIAG: Групп создано: {grouped}")
                            except Exception as e:
                        ...
                        i += 1
                # print(f"# DIAG: Всего групп создано: {grouped}")
            ...
            # Форматирование заголовков в остальных листах
            ...
            print("# DIAG: Сохраняю рабочую книгу...")
            wb.save(output_file)
...
def main():
    ...
    save_path = os.path.join(folder, 'WB&OZON NPS full.xlsx')
    ...
    # --- ГРУППИРОВКА И РАСЧЁТ NPS ПО ГОДАМ ---
