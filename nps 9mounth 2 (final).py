...
def split_dataframe_to_sheets(df, max_rows=1000000):
    """Разделяет DataFrame на несколько листов если строк больше max_rows"""
    sheets_data = {}
...
def add_totals_row(pivot, base_cols, months_order):
    """Добавляет итоговую строку в сводную таблицу"""
...
def npsview_one(excel_file, add_message):
    """Обрабатывает Excel файл: группирует комментарии и форматирует"""
    t = time.time()
    ...
    try:
        ...
        # ИЗМЕНЕНИЕ: УБРАНА ТОЛЬКО ФИЛЬТРАЦИЯ ПУСТЫХ КОММЕНТАРИЕВ
        # Комментарии оставляем как есть, даже пустые
        # Было:
        # df_comments = df_comments[df_comments["Комментарий"].notna()]
        # df_comments = df_comments[df_comments["Комментарий"].str.strip() != "-"]

        # Стало: оставляем ВСЕ комментарии, включая пустые
        ...
        # ФИЛЬТРАЦИЯ ПО ДАТАМ СОХРАНЯЕТСЯ - удаляем строки без дат
        ...
        with pd.ExcelWriter(output_file, engine="openpyxl", mode="w") as writer:
            ...
            # Сохраняем отфильтрованные по датам данные (но со всеми комментариями)
        ...
        if "по кодам" in wb.sheetnames:
            ...
            nps_cols = [col for col in range(1, max_col + 1)
                        if
                        sh.cell(row=1, column=col).value and str(sh.cell(row=1, column=col).value).startswith("NPS ")]
            for col in nps_cols:
...
def main():
    """Основная функция для обработки данных за 9 месяцев"""
    timer0 = time.perf_counter()
    folder = 'Исходники WB и Ozon'
    card_path = os.path.join(folder, 'Карточка товара.xlsx')
    # Изменено имя файла для 9 месяцев
    save_path = os.path.join(folder, 'WB&OZON NPS 9 месяцев.xlsx')
    ...
    print('Заполняю данные из Карточки товаров...')
    ...
    print('Рассчитываю NPS за 9 месяцев...')
    ...
    # ИЗМЕНЕНИЕ: Берем 9 месяцев вместо 3
    # Получаем все уникальные месяцы, сортируем по возрастанию и берем последние 9
    unique_months = sorted(df_final['_YM'].dropna().unique())

    # Если данных меньше 9 месяцев, берем все доступные
    if len(unique_months) < 9:
        print(f'Внимание: данных только за {len(unique_months)} месяцев. Будут использованы все доступные данные.')
        last_months = unique_months
    else:
        last_months = unique_months[-9:]  # Берем последние 9 месяцев

    # ФИЛЬТРАЦИЯ ПО ДАТАМ СОХРАНЯЕТСЯ - оставляем только данные за последние 9 месяцев
    mask = df_final['_YM'].isin(last_months)
    df_9mo = df_final[mask].copy()
    ...
    df_9mo['Месяц-год'] = df_9mo['Дата_dt'].apply(get_month_rus)
    months_order = (
        df_9mo
    ...
    print(f'Анализируем данные за {len(months_order)} месяцев: {", ".join(months_order)}')

    print('Формирую итоговый вид...')
    index_cols = ["Код продукта", "Продукт", "Поставщик", "Основной менеджер", "СТМ"]
    df_9mo[index_cols] = df_9mo[index_cols].fillna('')
    ...
    df_9mo.drop(columns=['Дата_dt', '_YM', 'Месяц-год'], inplace=True, errors='ignore')

    print('Сохраняю результат...')
    t9 = time.perf_counter()
    with pd.ExcelWriter(save_path) as writer:
        # Сохраняем данные за последние 9 месяцев (отфильтрованные по датам)
        general_sheets_data = split_dataframe_to_sheets(df_9mo)
        for sheet_name, sheet_data in general_sheets_data.items():
            sheet_data.to_excel(writer, sheet_name=sheet_name, index=False)
        pivot.to_excel(writer, sheet_name="по кодам", index=False)

    print(f'Готово! Сводная NPS за 9 месяцев сохранена: {save_path}')
