...
def main():
    ...
    save_path = os.path.join(folder, 'WB&OZON NPS 3 мес.60%.xlsx')
    ...
    # ===========================
    # ОТФИЛЬТРОВАТЬ И ДОБАВИТЬ ИТОГИ КАК НАДО
    # ===========================
    code_col_for_pivot = base_cols[0] if len(base_cols) > 0 else "Код продукта"
    to_drop_mask = (pivot['NPS Общий итог'] > 0.6) & (pivot[code_col_for_pivot] != 'ИТОГО')
    codes_to_delete = set(pivot.loc[to_drop_mask, code_col_for_pivot].dropna())
    pivot = pivot[~to_drop_mask].reset_index(drop=True)
    df_3mo = df_3mo[~df_3mo['Код продукта'].isin(codes_to_delete)].reset_index(drop=True)
    # Добавляем итоговую строку после всех фильтраций!
